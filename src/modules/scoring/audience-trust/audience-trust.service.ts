...
export function calculateOverallAudienceTrust(
  ...
  overallScore: number;
  breakdown: PlatformAudienceTrustResult[];
  brandOverSafetyAllScore: number;
} {
  let weightedSum = 0;
  ...
  for (const result of platformResults) {
    ...
    brandSafetyWeightedSum +=
      (result.breakdown.brandSafety ?? 0) * (weight / 100);
    totalWeight += weight;
  }

  const overallScore = totalWeight > 0 ? weightedSum : 0;
  const brandOverSafetyAllScore =
    brandSafetyWeightedSum > 0 ? brandSafetyWeightedSum : 0;
