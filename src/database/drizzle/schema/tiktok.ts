...
export const tiktokRawData = pgTable(
  "tiktok_raw_data",
  {
  ...
  (table) => ({
    linkIdx: index("tiktok_raw_link_idx").on(table.linkId),
  }),
);

export const tiktokScores = pgTable(
  "tiktok_scores",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    linkIdx: index("tiktok_score_link_idx").on(table.linkId),
  }),
);
