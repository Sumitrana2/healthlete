...
function extractEngagementMark(platform: string, raw: any): string | null {
  switch (platform) {
    ...
    case "twitter":
      return (
        raw?.report?.metrics?.tweets_number?.performance?.["90d"]?.mark ??
        raw?.report?.metrics?.tweets_number?.performance?.mark ??
        null
      );
    default:
      return null;
...
export function calculateEngagementQuality(
  ...
  const weightedAvg = (
    field: "avgEngagementRate" | "avgLikes" | "avgComments",
  ): number | null => {
    let weightedSum = 0;
    let totalWeight = 0;
  ...
  const engagementQualityScore =
    markTotalWeight > 0
      ? Math.round((markWeightedSum / markTotalWeight) * 100) / 100
      : 0;
