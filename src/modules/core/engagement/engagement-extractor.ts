// core/engagement/engagement-extractor.ts
...
function extractTwitterEngagement(raw: any): PlatformEngagementMetrics {
  return {
    avgEngagementRate:
      ...
      null,
    avgLikes:raw?.report?.metrics?.favorite_avg?.performance?.["180d"]?.value??
      raw?.report?.metrics?.favorite_avg?.performance?.["90d"]?.value ?? null,

    avgComments: raw?.report?.metrics?.reply_avg?.performance?.["180d"].value ??
      raw?.report?.metrics?.reply_avg?.performance?.["90d"]?.value ?? null,
  };
}
