...
export const youtubeRawData = pgTable(
  "youtube_raw_data",
  {
  ...
  (table) => ({
    linkIdx: index("youtube_raw_link_idx").on(table.linkId),
  })
);

export const youtubeScores = pgTable(
  "youtube_scores",
  {
    id: uuid("id").primaryKey().defaultRandom(),
  ...
  (table) => ({
    linkIdx: index("youtube_score_link_idx").on(table.linkId),
  })
);
